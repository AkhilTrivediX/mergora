...
const packageOutputRoot = resolve(packageRoot, "dist");
let strippedTypeOnlyStyleImports = 0;
for (const declaration of walk(packageOutputRoot).filter((path) => path.endsWith(".d.ts"))) {
  const source = readFileSync(declaration, "utf8");
  const normalized = source.replace(/^import\s+["']\.\/[^"']+\.css["'];\r?\n/gmu, "");
  if (normalized === source) continue;
  writeFileSync(declaration, normalized, "utf8");
  strippedTypeOnlyStyleImports += 1;
}

writeFileSync(
  resolve(packageOutputRoot, "style.d.ts"),
  [
...
console.log(
  `Copied ${String(copied)} generated UI package assets and normalized ${String(strippedTypeOnlyStyleImports)} declaration style imports.`,
);
