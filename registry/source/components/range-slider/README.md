...
RangeSlider keeps the shared Mergora measurement rail while giving the minimum and maximum thumbs persistent green and violet edge identities, including when they overlap. It inherits independently optional `intelligentMarks` and `showValueBubbles`. `announceCollisions` adds one polite localized status when adjacent thumbs meet. All three enhancements default to off; disabling them removes their DOM, event-driven messaging, and accessibility output rather than hiding a still-active feature.
...
Promotion requires generated artifacts, package/source parity, multi-thumb form and reset evidence, collision and overlapping-thumb identity tests, enabled/disabled collision-announcement evidence, localized accessible value text, intelligent-mark and value-bubble coverage, horizontal/vertical/RTL geometry, keyboard and drag cancellation coverage, narrow and forced-color review, and current manual assistive-technology sessions.
