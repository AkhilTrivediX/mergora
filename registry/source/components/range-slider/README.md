...
The collision policy is deliberately fixed to `clamp`. Thumbs may meet, but they never cross, swap indices, or exchange semantic identity. Values must arrive in non-decreasing order. Default labels identify the first and last thumbs as minimum and maximum; production use should supply domain-specific `thumbLabels`, such as “Minimum monthly salary” and “Maximum monthly salary.” Optional `names` are also count-aligned, non-empty, and distinct so a form never loses a boundary.
