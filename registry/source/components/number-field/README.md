...
The field's Mergora signature uses a literal white editing surface, strong ink boundary, precise tabular values, compact workbench controls, green pressed state, and violet focus treatment through semantic tokens. Its existing `scrub` control is an optional direct-manipulation advantage over an ordinary input. `statusRail="auto"` adds localized accepted-bound context to the input description, while `showCanonicalPreview` exposes the exact JavaScript/form number in a non-live machine-value rail. These enhancements are independent and off by default; when disabled they add no UI, event handler, description reference, or accessibility output.
...
Promotion requires generated artifacts, package/source parity, canonical form and reset evidence, status/canonical opt-in and disabled-absence evidence, locale and non-Latin digit parsing, IME and caret review, mobile and narrow geometry, RTL, forced colors, keyboard, scrub cancellation, and current manual assistive-technology sessions.
