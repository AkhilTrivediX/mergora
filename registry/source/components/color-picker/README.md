...
Precise Ink geometry, literal Canvas editing surfaces, Green selection, Violet focus seams, exact canonical text, and restrained corner treatment make the spatial control part of the same Mergora field family rather than a decorative palette.
...
`showPreview`, `showContrast`, and `swatches` are independent. Set either output flag to `false` or pass `swatches={[]}` to remove that enhancement's UI and accessibility output while the exact editor, channel controls, value ownership, and native serialization remain intact.
