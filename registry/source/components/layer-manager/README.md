...
Set `Layer.manageEnvironment={false}` only when a named external behavior engine already owns inerting and scroll prevention for that modal, as React Aria does for Mergora Dialog. The layer remains modal in the shared order and still blocks Escape from reaching lower layers; application `inert`, body styles, scroll locking, and their cleanup are all omitted. Setting `modal={false}` also removes modal environment behavior and modal stack membership while retaining ordinary layer ordering. The defaults remain `manageEnvironment={true}` and `modal={false}`, so existing consumers are unchanged. Keep coordinated portals on the shared `--mrg-component-layer-stack-base`; a consumer z-index override must not contradict registration order.

This separation of shared visual/Escape order from optional environment ownership is the Mergora advantage: independently authored overlays can coordinate without applying inerting or scroll prevention twice. Layer Manager adds no visible panel, role, label, live region, or focus movement in either mode.
